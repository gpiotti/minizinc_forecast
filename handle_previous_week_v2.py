...
def is_course_ok(prev_shift, prev_start, curr_type, curr_start): #checks if the shift from prev week could be used to substract a request at a timeblock
...
while i < len(previous_shifts):
        ...
        while prev_q > 0 and j < len(current_requests) -1 :
                ...
                while j < len(current_requests) -1 and prev_q > 0:
                        ...
                        if time_.tm_hour >= 2 or get_daysdiff( prev_start, curr_start) > 1 :
                                j += 1
                                continue
